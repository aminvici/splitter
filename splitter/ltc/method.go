...
func ParseBlock(data string) (*LTCBlockData, error) {
	startTime := time.Now()
	b := new(LTCBlockData)
	...
	log.Debug("splitter ltc: parse block %d, txs %d, elasped time %s", b.Block.Height, b.Block.TxCount, elaspedTime.String())
	return b, nil
}

func updateTransactionVersion(tx *service.Transaction, txVersion []int64, data *LTCBlockData) error {
	count := 0
	var sql, sqlUpdate, sqlUpdate2 string
	for k, v := range txVersion {
		id := data.Transactions[k].TxID
		if count == 0 {
			sqlUpdate = fmt.Sprintf("UPDATE ltc_transaction SET version=CASE tx_id WHEN '%s' THEN %d", id, v)
...
func UpdateBlock(data *LTCBlockData, tx *service.Transaction) error {
	height := data.Block.Height
	err := updateVOutIsUsed(height, tx)
	if err != nil {
		log.Error("splitter ltc index: %s calculation error", "UpdateSelectVInValueAndAddress")
		return err
	}
	err = updateAddressTable(data, tx)
	if err != nil {
		log.Error("splitter ltc index: %s calculation error", "UpdateAddressTable")
		return err
	}
	err = updateCoinbaseAddressCount(height, tx)
	if err != nil {
		log.Error("splitter ltc index: %s calculation error", "UpdateCoinbaseAddressCount")
		return err
	}
	return nil
}
...
func updateCoinbaseAddressCount(height int64, tx *service.Transaction) error {
	...
	sql := fmt.Sprintf("SELECT address FROM ltc_vout WHERE block_height=%d AND is_coinbase=1", height)
	...
	elaspedTime := time.Now().Sub(startTime)
	log.Debug("splitter ltc index: %s elasped %s", index, elaspedTime.String())
	return nil
}
...
func revertSelectVInValueAndAddress(height int64, tx *service.Transaction) error {
	...
	sql := fmt.Sprintf("UPDATE ltc_vout SET is_used=0 where id in (select b.id from ltc_vin a INNER JOIN ltc_vout b ON a.tx_id_origin=b.tx_id AND a.vout_num_origin=b.number AND a.block_height=%d)", height)
	affected, err := tx.Execute(sql)
	if err != nil {
		return err
	}
	elaspedTime := time.Now().Sub(startTime)
	log.Debug("splitter ltc index: %s affected %d elasped %s", index, affected, elaspedTime.String())
	return nil
}
...
func revertAddressTable(height int64, tx *service.Transaction) error {
	...
	sql := fmt.Sprintf("DELETE FROM ltc_address WHERE birth_timestamp=(SELECT timestamp FROM ltc_block WHERE height=%d)", height)
	affected1, err := tx.Execute(sql)
	if err != nil {
		return err
	}
	sql = fmt.Sprintf("select a.address address,a.value+b.value value FROM ltc_address a JOIN (SELECT address_origin address, SUM(value_origin) value FROM ltc_vin WHERE block_height=%d GROUP BY address_origin) b ON a.address=b.address", height)
	result1, err := tx.QueryString(sql)
	if err != nil {
		return err
	}
	for _, v := range result1 {
		sql := fmt.Sprintf("update ltc_address set value=%s where address='%s'", v["value"], v["address"])
		_, err := tx.Exec(sql)
		if err != nil {
			return err
		}
	}
	sql = fmt.Sprintf("select a.address address,a.value-b.value value FROM ltc_address a JOIN (SELECT address, SUM(value) value FROM ltc_vout WHERE block_height=%d GROUP BY address) b ON a.address=b.address", height)
	...
	elaspedTime := time.Now().Sub(startTime)
	log.Debug("splitter ltc index: %s affected %d %d %d elasped %s", index, affected1, len(result1), len(result2), elaspedTime.String())
	return nil
}
...
func revertCoinbaseAddressCount(height int64, tx *service.Transaction) error {
	...
	for _, v := range result {
		address := v["address"]
		sql = fmt.Sprintf("UPDATE ltc_mining SET coinbase_times=coinbase_times-1 WHERE address='%s'", address)
		affected, err := tx.Execute(sql)
		if err != nil {
			return err
		}
	...
	elaspedTime := time.Now().Sub(startTime)
	log.Debug("splitter ltc index: %s affected %d elasped %s", index, totalAffected, elaspedTime.String())
	return nil
}

func updateVInAddressAndValue(tx *service.Transaction, data *LTCBlockData) error {
	valueMap := make(map[string]int64)
	for k, v := range data.VIns {
		sql := fmt.Sprintf("select address,value from ltc_vout where tx_id='%s' and number=%d", v.TxIDOrigin, v.VOutNumberOrigin)
		result, err := tx.QueryString(sql)
		if err != nil {
			return err
		}
...
func updateVOutIsUsed(height int64, tx *service.Transaction) error {
	...
	sql := fmt.Sprintf("select a.* from ltc_vout as a inner join (select tx_id_origin,vout_num_origin from ltc_vin where block_height=%d) as b on a.tx_id=b.tx_id_origin and a.number=b.vout_num_origin", height)
	result, err := tx.QueryString(sql)
	if err != nil {
		return err
	}
	...
	sql = fmt.Sprintf("delete from ltc_vout where id in (select a.id from ltc_vout as a inner join (select tx_id_origin,vout_num_origin from ltc_vin where block_height=%d) as b on a.tx_id=b.tx_id_origin and a.number=b.vout_num_origin)", height)
	_, err = tx.Exec(sql)
	if err != nil {
		return err
	}
	...
	log.Debug("ltc calculate index: insert vOut affected %d done", affected)
	return nil
}

// 插入新的账户地址，更新旧地址的最后交易时间，重新计算余额
func updateAddressTable(data *LTCBlockData, tx *service.Transaction) error {
	height := data.Block.Height
	...
	sql := fmt.Sprintf("SELECT * FROM ltc_address where address in (SELECT address FROM ltc_vout WHERE block_height=%d UNION SELECT address_origin FROM ltc_vin WHERE block_height=%d)", height, height)
	result, err := tx.QueryString(sql)
	if err != nil {
		return err
	}
	...
	sql = fmt.Sprintf("DELETE FROM ltc_address where address in (SELECT address FROM ltc_vout WHERE block_height=%d UNION SELECT address_origin FROM ltc_vin WHERE block_height=%d)", height, height)
	_, err = tx.Exec(sql)
	if err != nil {
		return err
	}
...
// 更新标记block所属矿池
func GetBlockMiner(data *LTCBlockData, tx *service.Transaction) error {
