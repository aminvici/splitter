...
func NewSplitter(cfg *SplitterConfig) (*LTCSplitter, error) {
	var err error
	s := new(LTCSplitter)
	s.cfg = cfg
...
func (s *LTCSplitter) Stop() {
	s.cronWorker.Stop()
}

func (s *LTCSplitter) CheckBlock(curBlock *LTCBlockData) (bool, int64) {
	...
	if len(preBlock) != 1 {
		var start, end int64
		log.Warn("splitter ltc: can not find previous block %d", curBlock.Block.Height-1)
		...
		if err != nil {
			...
			log.Debug("splitter ltc: get latest block %d from database", start)
			if curBlock.Block.Height > start+int64(s.cfg.MaxBatchBlock) {
				end = start + int64(s.cfg.MaxBatchBlock)
			}
			log.Debug("splitter ltc: get block range from %d to %d", start+1, end)
	...
	if preBlock[0].Hash != curBlock.Block.PreviousHash {
		log.Warn("splitter ltc: block %d is revert", curBlock.Block.Height-1)
	...
	log.Debug("splitter ltc: check block %d pass", curBlock.Block.Height)
	return true, height
}

// revert block by height
func (s *LTCSplitter) RevertBlock(height int64, tx *service.Transaction) error {
	startTime := time.Now()
	...
	//revert block table
	sql := fmt.Sprintf("DELETE from ltc_block WHERE height = %d", height)
	affected, err := tx.Execute(sql)
	if err != nil {
		return err
	}
	log.Debug("splitter ltc: revert block %d from ltc_block table, affected", height, affected)
	//revert transaction table
	sql = fmt.Sprintf("delete from ltc_transaction where block_height = %d", height)
	affected, err = tx.Execute(sql)
	if err != nil {
		return err
	}
	log.Debug("splitter ltc: revert block %d from ltc_transaction table, affected", height, affected)
	//revert vin table
	sql = fmt.Sprintf("DELETE FROM ltc_vin WHERE block_height = %d", height)
	affected, err = tx.Execute(sql)
	if err != nil {
		return err
	}
	log.Debug("splitter ltc: revert block %d from ltc_vin table, affected", height, affected)
	//revert vout table
	sql = fmt.Sprintf("DELETE FROM ltc_vout WHERE block_height = %d", height)
	affected, err = tx.Execute(sql)
	if err != nil {
		return err
	}
	log.Debug("splitter ltc: revert block %d from ltc_vout table, affected", height, affected)

	elaspedTime := time.Now().Sub(startTime)
	log.Debug("splitter ltc: revert block %d elasped %s", height, elaspedTime.String())
	return nil
}

func (s *LTCSplitter) Start() {
	//start kafka consumer
	err := s.cfg.Consumer.Start(s.cfg.Topic)
	if err != nil {
		log.Error("splitter ltc: consumer start error")
		log.DetailError(err)
		return
	}

	log.Debug("splitter ltc: consumer start topic %s", s.cfg.Topic)
	log.Debug("splitter ltc: database enable is %v", s.cfg.DatabaseEnable)

	//start cron worker
	err = s.cronWorker.Start()
	if err != nil {
		log.Error("splitter ltc: cron worker start error")
		log.DetailError(err)
		return
	}
	...
	for {
		...
		case message := <-s.cfg.Consumer.MessageChannel():
			log.Debug("splitter ltc: topic %s receive data on partition %d offset %d length %d",
		...
		START:
			...
			if s.cfg.JSONSchemaValidationEnable {
				...
				if err != nil {
					log.Error("splitter ltc: json schema valid error")
				}
				if !ok {
					log.Warn("splitter ltc: json schema valid failed")
				}
			}
			...
			if err != nil {
				...
				log.Error("splitter ltc: block parse error, retry after 5s")
				log.DetailError(err)
			...
			if _, ok := s.missedBlockList[data.Block.Height]; !ok {
				log.Debug("splitter ltc: checking block %d", data.Block.Height)
				ok, height := s.CheckBlock(data)
				if data.Block.Height != 0 && !ok {
					log.Debug("splitter ltc: block check failed, expected height %d, this block height %d", height, data.Block.Height)
					continue
				}
			} else {
				log.Debug("splitter ltc: block %d is missed", data.Block.Height)
			...
			if s.cfg.DatabaseEnable {
				...
				if err != nil {
					log.Error("splitter ltc: block %d save error, retry after 5s", data.Block.Height)
					log.DetailError(err)
					time.Sleep(time.Second * 5)
					goto START
				} else {
					log.Info("splitter ltc: block %d save success", data.Block.Height)
...
// check json schema
func (s *LTCSplitter) jsonSchemaValid(data string) (bool, error) {
	startTime := time.Now()
	dataLoader := gojsonschema.NewStringLoader(data)
	result, err := gojsonschema.Validate(s.jsonSchemaLoader, dataLoader)
	if err != nil {
		log.Error("splitter ltc: json schema validation error")
		log.DetailError(err)
		return false, err
	}
	if !result.Valid() {
		for _, err := range result.Errors() {
			log.Error("splitter ltc: data invalid %s", strings.ToLower(err.String()))
			return false, nil
		}
	...
	elaspedTime := time.Now().Sub(startTime)
	log.Debug("splitter ltc: json schema validation elasped %s", elaspedTime)
	return true, nil
}

func (s *LTCSplitter) SaveBlock(data *LTCBlockData) error {
	startTime := time.Now()
	...
	if data.Block.Height == 0 {
		...
		if len(blocks) != 0 {
			log.Warn("splitter ltc: block %d has been stored", data.Block.Height)
			_ = tx.Rollback()
			return nil
		}
	}
	if data.Block.Height != 0 && has {
		if blockTemp.Hash == data.Block.Hash {
			...
			if blocks[0].Height-data.Block.Height > 6 {
				log.Warn("splitter ltc: block %d reverted is too old", data.Block.Height)
				_ = tx.Rollback()
				return nil
			}
	...
	if config.SplitterConfig.DatabaseLTCSetting.Type != "postgres" {
		sql := fmt.Sprintf("UPDATE ltc_block SET version='%d' WHERE height='%d'", version, data.Block.Height)
	...
	log.Debug("splitter ltc: block write %d rows", affected)
	...
	log.Debug("splitter ltc: vout write %d rows", affected)
	...
	log.Debug("splitter ltc: transaction write %d rows", affected)
	...
	log.Debug("splitter ltc: vin write %d rows", affected)
	...
	log.Debug("splitter ltc: block %d write done elasped: %s", data.Block.Height, elaspedTime.String())
	return nil
}
