...
func (h *rpcHandler) SendBlock(number int64) error {
	...
	_, err := h.client.Call("eth_sendBlockByNumber", hexNumber, true)
	if err != nil {
		return err
	}
	return nil
...
func (h *rpcHandler) SendBatchBlock(startNumber, endNumber int64) error {
	defer stats.Add(MetricRPCCall, 1)
	hexStartNumber := fmt.Sprintf("%#x", startNumber)
	hexEndNumber := fmt.Sprintf("%#x", endNumber)
	_, err := h.client.Call("eth_sendBatchBlockByNumber", hexStartNumber, hexEndNumber)
	if err != nil {
		return err
	}
	return nil
